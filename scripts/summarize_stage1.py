...
def _expected_stream_dates(
    ...
    *,
    ...
    train_start = max(dataset.splits.train.start or 0, window_size - 1)
    if stream == "train_fit":
        indices = slice(train_start, dataset.splits.train.stop, train_stride)
    elif stream == "train_dense":
        indices = slice(train_start, dataset.splits.train.stop)
    elif stream == "validation":
        indices = dataset.splits.validation
    elif stream == "test":
...
def _validate_manifest_and_streams(
    ...
    train_stride = hyperparameters.get("train_stride")
    if not isinstance(window_size, int) or not isinstance(train_stride, int):
        raise ValueError(f"{seed_dir}/manifest.json lacks integer window/stride metadata")
    ...
    for stream, filename in PREDICTION_FILENAMES.items():
        ...
        expected_dates = _expected_stream_dates(
            dataset,
            stream,
        ...
        expected_cadence = train_stride if stream == "train_fit" else 1
        expected_metadata = {
            "filename": filename,
            "rows": len(dates),
...
def summarize_stage1(run_dir: Path, data_path: Path) -> dict[str, Any]:
    ...
    if run_kind == "full":
        selected = min(
            per_seed,
            key=lambda row: (float(row["metrics"]["validation_loss"]), row["seed"]),
        )
        summary["stage1_selection"] = {
            ...
            "validation_loss": selected["metrics"]["validation_loss"],
        }
