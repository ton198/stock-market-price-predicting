...
Do macroeconomic and geopolitical indicators improve trading performance in neural forecasting models, and can evolutionary optimization further improve these results compared to traditional strategies?
...
\noindent To improve trading performance, we will use an evolutionary algorithm to optimize trading decisions based on model predictions and additional market indicators.\\

\noindent In this framework, a trading strategy is defined using a set of parameters, including weights and thresholds. At each time step $t$, we compute a decision score:

\[
s_t = w_1 r_t + w_2 x_t,
\]

\noindent Here $r_t$ is the predicted return from previous model and $x_t$ represents additional indicators such as geopolitical or macroeconomic signals.\\

\noindent The trading action is then determined by comparing the score with predefined thresholds:

\[
\begin{cases}
\text{Buy} & \text{if } s_t > \theta_{\text{buy}}, \\
\text{Sell} & \text{if } s_t < \theta_{\text{sell}}, \\
\text{Hold} & \text{otherwise}.
\end{cases}
\]

\noindent The final goal of the evolutionary algorithm is to find the optimal parameters $\{w_1, w_2, \theta_{\text{buy}}, \theta_{\text{sell}}\}$ that can earn more money. 
